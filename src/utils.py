...
def get_props_from_os(list_of_buildings):
    ...
    for i in range(len(list_of_buildings)):
        ...
        for j in range(len(uprn_array)):
            ...
            #     {"Accept": "application/json", "Authorization": f"Basic {TOKEN}"},
            #     f'uprn={uprn_array[j]["uprn"]}',
            # )
            # new_prop.epc_rating = (
            #     epc_result["rows"][0]["current-energy-rating"]
            #     if epc_result
            #     else "Not found"
            # )
            # new_prop.epc_score = (
            #     epc_result["rows"][0]["current-energy-efficiency"]
            #     if epc_result
            #     else "Not found"
            # )
            # new_prop.address = (
            #     f'{epc_result["rows"][0]["address"]}, {epc_result["rows"][0]["postcode"]}'
            #     if epc_result
            #     else "Not found"
            # )
