...
def test_property_has_uprn_from_dummy_data():
    dummy_property = Property(dummy_data['rows'][0]['uprn'], None, None, None, None)
    assert dummy_property.uprn == '200002791'
...
def test_property_has_correct_attributes_from_api_call():
    dummy_property = Property(epc_test_property['uprn'], epc_test_property['current-energy-rating'], epc_test_property['current-energy-efficiency'], epc_test_property['address'], epc_test_property['postcode'])
    assert dummy_property.epc_rating == 'D'
    assert dummy_property.uprn == '200002791'
